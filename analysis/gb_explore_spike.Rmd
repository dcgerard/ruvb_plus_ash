...
saveRDS(dout, file = "../output/gbout/gender.sm_spike.RDS")
...
colnames(X) <- c("Intercept", "Gender")
ctl <- gender.bgnmsm$spikectl
onsex <- gender.bgnmsm$xchrom | gender.bgnmsm$ychrom + gender.bgnmsm$xychrom
dout <- analyze_dat(Y = Y, X = X, ctl = ctl)
saveRDS(dout, file = "../output/gbout/gender.bgnmsm_spike.RDS")
enrlong <- gather(dout$enrdat, key = "Method", value = "Num", RUV2:RUVB) %>%
    filter(Index <= 200)
ggplot(data = enrlong, mapping = aes(x = Index, y = Num, color = Method, lty = Method)) +
  geom_line() +
...
colnames(X) <- c("Intercept", "Gender")
ctl <- gender.bgnmsmls$spikectl
onsex <- gender.bgnmsmls$xchrom | gender.bgnmsmls$ychrom + gender.bgnmsmls$xychrom
dout <- analyze_dat(Y = Y, X = X, ctl = ctl)
saveRDS(dout, file = "../output/gbout/gender.bgnmsmls_spike.RDS")
enrlong <- gather(dout$enrdat, key = "Method", value = "Num", RUV2:RUVB) %>%
    filter(Index <= 200)
ggplot(data = enrlong, mapping = aes(x = Index, y = Num, color = Method, lty = Method)) +
  geom_line() +
...
colnames(X) <- c("Intercept", "Gender")
ctl <- alzheimers.sm$spikectl
onsex <- alzheimers.sm$xchrom | alzheimers.sm$ychrom + alzheimers.sm$xychrom
dout <- analyze_dat(Y = Y, X = X, ctl = ctl)
saveRDS(dout, file = "../output/gbout/alzheimers.sm_spike.RDS")
enrlong <- gather(dout$enrdat, key = "Method", value = "Num", RUV2:RUVB) %>%
    filter(Index <= 200)
ggplot(data = enrlong, mapping = aes(x = Index, y = Num, color = Method, lty = Method)) +
  geom_line() +
...
colnames(X) <- c("Intercept", "Gender")
ctl <- alzheimers.bgnmsm$spikectl
onsex <- alzheimers.bgnmsm$xchrom | alzheimers.bgnmsm$ychrom + alzheimers.bgnmsm$xychrom
dout <- analyze_dat(Y = Y, X = X, ctl = ctl)
saveRDS(dout, file = "../output/gbout/alzheimers.bgnmsm_spike.RDS")
enrlong <- gather(dout$enrdat, key = "Method", value = "Num", RUV2:RUVB) %>%
    filter(Index <= 200)
ggplot(data = enrlong, mapping = aes(x = Index, y = Num, color = Method, lty = Method)) +
  geom_line() +
