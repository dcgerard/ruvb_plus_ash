## Run ASH as if alpha = 0 even though simulated under alpha = 1.
...
one_rep <- function(new_params, current_params) {
  ...
  method_list$ruv3 <- ruv3_simp(Y = Y, X = X, num_sv = num_sv,
                                control_genes = control_genes)
  method_list$ruv4 <- ruv4_simp(Y = Y, X = X, num_sv = num_sv,
                                control_genes = control_genes)
  method_list$cate <- cate_simp(Y = Y, X = X, num_sv = num_sv,
                                control_genes = control_genes)
  ...
  ash_list$mouthwash_norm <- mouthwash(Y = Y, X = X, num_sv = num_sv,
                                       likelihood = "normal", alpha = 0, scale_var = FALSE)
  ash_list$mouthwash_t <- mouthwash(Y = Y, X = X, num_sv = num_sv,
                                    likelihood = "t", alpha = 0, scale_var = FALSE)
  ash_list$backwash    <- backwash(Y = Y, X = X, num_sv = num_sv, alpha = 0, scale_var = FALSE)
  ash_list$mouthwash_norm_sv <- mouthwash(Y = Y, X = X, num_sv = num_sv,
                                       likelihood = "normal", alpha = 0, scale_var = TRUE)
  ash_list$mouthwash_t_sv <- mouthwash(Y = Y, X = X, num_sv = num_sv,
                                    likelihood = "t", alpha = 0, scale_var = TRUE)
  ash_list$backwash_sv    <- backwash(Y = Y, X = X, num_sv = num_sv, alpha = 0, scale_var = TRUE)
...
saveRDS(cbind(par_vals, sout), "../output/alpha_1_sims_out/sims_out_asifalpha0.RDS")
