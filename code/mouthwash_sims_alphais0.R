...
one_rep <- function(new_params, current_params) {
  ...
  method_list$olsc <- ctl_adjust(method_list$ols, control_genes = control_genes)
  method_list$olsm <- mad_adjust(method_list$ols)

  ## control gene methods --------------------------------------------------
  method_list$ruv2_o <- ruv2_simp(Y = Y, X = X, num_sv = num_sv, control_genes = control_genes)
  method_list$ruv2  <- limma_adjust(method_list$ruv2_o)
  method_list$ruv2c <- ctl_adjust(method_list$ruv2, control_genes = control_genes)
  method_list$ruv2m <- mad_adjust(method_list$ruv2)

  method_list$ruv3 <- ruv3_limma_pre(Y = Y, X = X, num_sv = num_sv,
                                        control_genes = control_genes)
  method_list$ruv3c <- ruv3_limma_pre_adjust(Y = Y, X = X, num_sv = num_sv,
                                             control_genes = control_genes)
  method_list$ruv3m <- mad_adjust(method_list$ruv3)

  method_list$ruv4_o <- ruv4_simp(Y = Y, X = X, num_sv = num_sv, control_genes = control_genes)
  method_list$ruv4  <- limma_adjust(method_list$ruv4_o)
  method_list$ruv4c <- ctl_adjust(method_list$ruv4, control_genes = control_genes)
  method_list$ruv4m <- mad_adjust(method_list$ruv4)

  method_list$cate  <- cate_limma(Y = Y, X = X, num_sv = num_sv,
                                     control_genes = control_genes)
  method_list$catem <- mad_adjust(method_list$cate)
  method_list$catec <- ctl_adjust(method_list$cate, control_genes = control_genes)

  ## non control gene methods ----------------------------------------------
  method_list$sva  <- sva(Y = Y, X = X, num_sv = num_sv)
  method_list$svac <- ctl_adjust(method_list$sva, control_genes = control_genes)
  method_list$svam <- mad_adjust(method_list$sva)

  method_list$caterr  <- cate_rr(Y = Y, X = X, num_sv = num_sv, calibrate = FALSE)
  method_list$caterrc <- ctl_adjust(method_list$caterr, control_genes = control_genes)
  method_list$caterrm <- mad_adjust(method_list$caterr)
  ...
  ash_list$mouthwash_norm_unif <- mouthwash(Y = Y, X = X, num_sv = num_sv,
                                            likelihood = "normal",
                                            alpha = 0, scale_var = TRUE,
                                            mixing_dist = "sym_uniform")
  ash_list$mouthwash_t <- mouthwash(Y = Y, X = X, num_sv = num_sv,
                                    likelihood = "t", alpha = 0, scale_var = TRUE)
  ash_list$mouthwash_norm_alpha1 <- mouthwash(Y = Y, X = X, num_sv = num_sv,
                                              likelihood = "normal",
                                              alpha = 1, scale_var = TRUE,
                                              var_inflate_pen = 0.01)
  ash_list$mouthwash_norm_unif_alpha1 <- mouthwash(Y = Y, X = X, num_sv = num_sv,
                                                   likelihood = "normal",
                                                   alpha = 1, scale_var = TRUE,
                                                   mixing_dist = "sym_uniform",
                                                   var_inflate_pen = 0.01)
  ash_list$mouthwash_t_alpha1 <- mouthwash(Y = Y, X = X, num_sv = num_sv,
...
saveRDS(cbind(par_vals, sout), "../output/alpha_1_sims_out/sims_out_alpha0.RDS")
